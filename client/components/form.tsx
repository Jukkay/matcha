...
export const Helper = ({ helper, focus }: IHelper) => {
  ...
  return focus ? (
    <div className="help mt-3"><div className="help">{helper}</div></div>
  ) : (
    <div className="help my-6"><div className="help is-sr-only">{helper}</div></div>
  );
};
...
export const Notification = ({
  ...
  return notificationState ? (
    ...
    <div className="notification is-danger my-5 is-sr-only" onClick={handleClick}>
      <button className="delete"></button>
      {notificationText}
    </div>
  );
